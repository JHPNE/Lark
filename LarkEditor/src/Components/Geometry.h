...
private:
  ...
  void SerializeMesh(const content_tools::mesh& mesh, tinyxml2::XMLElement* parentElement,
    ...
    if (!mesh.vertices.empty()) {
      ...
      SerializerUtils::WriteAttribute(verticesElement, "count", (uint32_t)mesh.vertices.size());
  ...
  bool DeserializeMesh(content_tools::mesh& mesh, const tinyxml2::XMLElement* meshElement,
    ...
    if (verticesElement) {
      ...
      for (auto vElement = verticesElement->FirstChildElement("V");
           vElement;
    ...
    if (indicesElement) {
      ...
      if (indexText) {
        ...
        while (pos < indexStr.length()) {
          ...
          if (nextPos == std::string::npos) nextPos = indexStr.length();
