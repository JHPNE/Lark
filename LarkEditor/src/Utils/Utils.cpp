...
void Utils::SetEnvVar(const std::string &name, const std::string &value)
{
    try
    {
        ...
        {
            std::cerr << "Failed to set environment variable for current process: " << name
                      << std::endl;
            return;
        }
        ...
        {
            result = RegSetValueExA(hKey, name.c_str(), 0, REG_SZ, (const BYTE *)value.c_str(),
            ...
            if (result == ERROR_SUCCESS)
            {
                ...
                std::cout << "Successfully set persistent environment variable: " << name
                          << std::endl;
            }
            else
            {
...
std::string Utils::GetEnvVar(const std::string &name)
{
    try
    {
        ...
        if (value == nullptr)
        {
            ...
            if (RegOpenKeyExA(HKEY_CURRENT_USER, "Environment", 0, KEY_READ, &hKey) ==
                ERROR_SUCCESS)
            {
                if (RegQueryValueExA(hKey, name.c_str(), nullptr, nullptr, (LPBYTE)buffer,
            ...
            if (name == "LARK_ENGINE" && ShouldAutoSetup())
            {
                ...
                if (fs::exists(defaultPath) &&
                    fs::exists(defaultPath / "LarkEditor/ProjectTemplates"))
                {
...
void Utils::SaveToShellProfile(const std::string &name, const std::string &value)
{
    ...
    std::vector<std::string> profiles = {std::string(home) + "/.bashrc",
...
fs::path Utils::GetDefaultEnginePath()
{
    ...
    std::vector<fs::path> possiblePaths = {fs::current_path(),               // Current directory
