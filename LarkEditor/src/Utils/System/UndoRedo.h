...
class UndoRedoAction : public IUndoRedo
{
  public:
    ...
    UndoRedoAction(std::function<void()> undo, std::function<void()> redo, const std::string &name);

    template <typename T>
    UndoRedoAction(const std::string &property, T *instance,
                   const typename T::value_type &undoValue, const typename T::value_type &redoValue,
