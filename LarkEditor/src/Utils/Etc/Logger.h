...
struct LogMessage
{
    ...
    LogMessage(MessageType type, const std::string &msg, const std::string &file,
               const std::string &caller, int line)
        : time(std::chrono::system_clock::now()), type(type), message(msg), file(file),
...
class Logger
{
  public:
    ...
    void Log(MessageType type, const std::string &message, const std::string &file = "",
             const std::string &caller = "", int line = 0);
    void Clear();
