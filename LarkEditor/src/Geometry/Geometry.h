...
class Geometry : public Asset
{
  public:
    ...
    bool FromRawData(const u8 *data, size_t size)
    {
        ...
        for (u32 lod_idx = 0; lod_idx < lod_count; ++lod_idx)
        {
            ...
            for (u32 mesh_idx = 0; mesh_idx < mesh_count; ++mesh_idx)
            {
                ...
                printf("[Geometry::FromRawData] Mesh '%s': vertex_count=%u, index_count=%u, "
                       "lod_threshold=%.2f\n",
                ...
                if (vertex_count > 0)
                {
                    ...
                    {
                        printf("[Debug] Vert %u: Pos(%.3f, %.3f, %.3f), Normal(%.3f, %.3f, %.3f), "
                               "UV(%.3f, %.3f)\n",
                               i, mesh.vertices[i].position.x, mesh.vertices[i].position.y,
                               mesh.vertices[i].position.z, mesh.vertices[i].normal.x,
    ...
    static std::unique_ptr<Geometry> LoadGeometry(const char *filename)
    {
        ...
        if (!LoadObj(filename, &scene_data))
            return nullptr;
    ...
    static std::unique_ptr<Geometry> CreatePrimitive(content_tools::PrimitiveMeshType type,
                                                     const f32 *size = nullptr,
                                                     const u32 *segments = nullptr, u32 lod = 0)
    {
        ...
        if (segments)
            memcpy(init_info.segments, segments, sizeof(init_info.segments));
        init_info.lod = lod;
    ...
    static void randomModificationVertexes(uint32_t id, uint32_t vertexCount,
                                           std::vector<glm::vec3> old_vertices)
    {
