...
std::shared_ptr<Project> Project::Create(const std::string &name, const fs::path &path,
                                         const ProjectTemplate &tmpl)
{
    try
    {
        ...
        try
        {
            fs::copy_file(tmpl.GetIconPath(), hiddenDir / "Icon.png",
                          fs::copy_options::overwrite_existing);
            fs::copy_file(tmpl.GetScreenshotPath(), hiddenDir / "Screenshot.png",
                          fs::copy_options::overwrite_existing);
        }
        catch (const fs::filesystem_error &e)
        {
            Logger::Get().Log(MessageType::Error,
                              "Failed to copy template files: " + std::string(e.what()));
            return nullptr;
        }
...
std::shared_ptr<Project> Project::Load(const fs::path &projectFile)
{
    try
    {
        ...
        {
            Logger::Get().Log(MessageType::Error,
                              "Failed to load project file: " + projectFile.string());
            return nullptr;
        }
        ...
        {
            Logger::Get().Log(MessageType::Error,
                              "Failed to deserialize project: " + projectFile.string());
            return nullptr;
        }
...
Project::Project(const std::string &name, const fs::path &path)
    : m_name(name), m_path(path), m_activeScene(nullptr), m_isModified(false)
{
}
...
std::shared_ptr<Scene> Project::AddScene(const std::string &sceneName)
{
    ...
    if (scene)
    {
        ...
        auto action = std::make_shared<UndoRedoAction>(
            ...
            [this, sceneId]() { RemoveSceneInternal(sceneId); },
            // redo function - adds scene with same name
            [this, name]() { AddSceneInternal(name); }, "Add Scene: " + name);
...
bool Project::RemoveScene(uint32_t sceneId)
{
    ...
    if (RemoveSceneInternal(sceneId))
    {
        auto action = std::make_shared<UndoRedoAction>(
            ...
            [this, sceneName]() { AddSceneInternal(sceneName); },
            // Redo function - removes the scene again
            [this, sceneId]() { RemoveSceneInternal(sceneId); }, "Remove Scene: " + sceneName);
        GlobalUndoRedo::Instance().GetUndoRedo().Add(action);
        return true;
    }
    return false;
...
std::shared_ptr<Scene> Project::GetScene(uint32_t sceneId) const { return GetSceneById(sceneId); }
    ...
    for (const auto &scene : m_scenes)
    {
        ...
        for (const auto &entity : scene->GetEntities())
        {
            ...
            for (const auto &[compType, compPtr] : entity->GetAllComponents())
            {
                ...
                {
                    auto compName =
                        Component::ComponentTypeToString(compType); // Implement this function
...
bool Project::Deserialize(const tinyxml2::XMLElement *element, SerializationContext &context)
{
    ...
    for (auto sceneElement = scenesElement->FirstChildElement("Scene"); sceneElement;
         sceneElement = sceneElement->NextSiblingElement("Scene"))
    {
        ...
        // Load entities
        for (auto entityElement = sceneElement->FirstChildElement("Entity"); entityElement;
             entityElement = entityElement->NextSiblingElement("Entity"))
        {
            ...
            for (auto *compElement = entityElement->FirstChildElement(); compElement;
    ...
    if (scriptFile.is_open())
    {
        ...
        if (RegisterScript(scriptName))
        {
            Logger::Get().Log(MessageType::Info,
                              "Created and registered script: " + scriptPath.string());
        }
        else
        {
            Logger::Get().Log(MessageType::Error,
                              "Failed to register script: " + scriptPath.string());
            return false;
        }
    }
    else
