...
std::vector<std::shared_ptr<ProjectTemplate>>
ProjectTemplate::LoadTemplates(const fs::path &templatePath)
{
...
std::shared_ptr<ProjectTemplate> ProjectTemplate::LoadFromFile(const fs::path &templateFile)
{
    try
    {
        ...
        if (!fs::exists(tmpl->m_iconPath) || !fs::exists(tmpl->m_screenshotPath) ||
    ...
    {
        Logger::Get().Log(MessageType::Error, "Failed to load template: " + std::string(e.what()));
        return nullptr;
    }
    return nullptr;
}
