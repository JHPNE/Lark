...
std::unique_ptr<GeometryRenderer::LODGroupBuffers>
GeometryRenderer::CreateBuffersFromGeometry(content_tools::scene *scene)
{
    if (!scene)
    {
...
std::unique_ptr<GeometryRenderer::LODGroupBuffers>
GeometryRenderer::UpdateBuffersfromGeometry(content_tools::scene *scene,
                                           ...
                                           float distanceToCamera)
{
    ...
    glUniformMatrix4fv(glGetUniformLocation(m_basicShader, "model"), 1, GL_FALSE,
                       glm::value_ptr(model));
    glUniformMatrix4fv(glGetUniformLocation(m_basicShader, "view"), 1, GL_FALSE,
                       glm::value_ptr(view));
    glUniformMatrix4fv(glGetUniformLocation(m_basicShader, "projection"), 1, GL_FALSE,
...
std::shared_ptr<GeometryRenderer::MeshBuffers>
GeometryRenderer::CreateMeshBuffers(const content_tools::mesh &mesh)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, mesh.vertices.size() * sizeof(content_tools::vertex),
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, stride,
                          (void *)offsetof(content_tools::vertex, position));

    glEnableVertexAttribArray(1); // Normal
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, stride,
                          (void *)offsetof(content_tools::vertex, normal));

    glEnableVertexAttribArray(2); // UV
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, stride,
                          (void *)offsetof(content_tools::vertex, uv));
...
{
    if (!meshBuffers || !meshBuffers->vao)
        return;
    ...
    glUniform3f(glGetUniformLocation(m_basicShader, "objectColor"), 0.8f, 0.8f,
                0.9f); // Light blue-gray for faces
    glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(meshBuffers->indexCount),
                   meshBuffers->indexType, 0);

    // Draw edges
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    glLineWidth(1.0f);
    glUniform3f(glGetUniformLocation(m_basicShader, "objectColor"), 0.2f, 0.2f,
                0.3f); // Dark blue-gray for edges
