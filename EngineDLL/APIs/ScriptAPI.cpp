...
extern "C"
{
    ...
    {
      public:
        explicit PythonScriptWrapper(lark::game_entity::entity entity) : entity_script(entity) {}
    };
    ...
    ENGINE_API script::detail::script_creator GetScriptCreator(const char *name)
    {
        ...
        return script::detail::get_script_creator(script::detail::string_hash()(name));
    }
    ...
    ENGINE_API bool RegisterScript(const char *script_name)
    {
        ...
        bool success = lark::script::detail::register_script(
            lark::script::detail::string_hash()(script_name), creator);
