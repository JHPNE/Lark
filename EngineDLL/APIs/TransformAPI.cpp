...
extern "C"
{
    ENGINE_API bool SetEntityTransform(lark::id::id_type entity_id,
                                       const transform_component &transform)
    {
        ...
        transform_comp.set_position(
            math::v3(transform.position[0], transform.position[1], transform.position[2]));
        ...
        transform_comp.set_scale(
            math::v3(transform.scale[0], transform.scale[1], transform.scale[2]));
        return true;
    }

    ENGINE_API bool GetEntityTransform(lark::id::id_type entity_id,
                                       transform_component *out_transform)
    {
        if (!engine::is_entity_valid(entity_id) || !out_transform)
            return false;
