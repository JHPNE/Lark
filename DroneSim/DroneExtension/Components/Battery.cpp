...
namespace lark::battery {
  namespace {
    struct battery_data : public lark::drone_data::BatteryBody {
      bool is_valid{ false };
      drone_entity::drone_id drone_id{};
    };

    util::vector<battery_data> fuselages;
    util::vector<id::id_type> id_mapping;
    util::vector<id::generation_type> generations;
    std::deque<battery_id> free_ids;

    bool exists(battery_id id) {
  ...
  drone_component create(init_info info, drone_entity::entity entity) {
    ...
    if (free_ids.size() > id::min_deleted_elements) {
      ...
      assert(!exists(id));
      free_ids.pop_front();
      id = battery_id{ id::new_generation(id) };
      ++generations[id::index(id)];
    } else {
      id = battery_id{ (id::id_type)id_mapping.size() };
    ...
    static_cast<lark::drone_data::BatteryBody&>(data) = info;
