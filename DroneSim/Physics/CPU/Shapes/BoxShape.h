...
public:
  ...
  float ComputeMass() const override { return 0.0f; }
  ...
  glm::mat3 ComputeLocalInertiaTensor() const override {
    ...
    return glm::mat3((y2 + z2) / 3.0f, 0.0f, 0.0f, 0.0f, (x2 + z2) / 3.0f, 0.0f,
                     0.0f, 0.0f, (x2 + y2) / 3.0f);
  }

  glm::vec3 ComputeLocalCentroid() const override { return glm::vec3(0.0f); }

  glm::vec3 Support(const glm::vec3 &direction) const {
    return glm::vec3(direction.x > 0.0f ? m_halfExtents.x : -m_halfExtents.x,
