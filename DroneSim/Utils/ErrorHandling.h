...
public:
    ...
    explicit ValidationError(
        const std::string& message,
        const std::uint32_t code = ERROR_BASE,
        const ErrorSeverity severity = ErrorSeverity::MODERATE,
        const SourceLocation& location = MAKE_SOURCE_LOCATION()
    ) noexcept(false) : 
