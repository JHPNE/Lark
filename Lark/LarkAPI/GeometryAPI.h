...
inline bool update_dynamic_mesh(game_entity::entity_id id,
                                const std::vector<math::v3> &new_positions)
{
    if (!id::is_valid(id) || !game_entity::is_alive(id))
        return false;
    ...
    if (geometry.is_dynamic())
    {
        ...
        if (!geometry.update_vertices(new_positions))
            return false;
...
{
    if (!id::is_valid(id) || !game_entity::is_alive(id) || !data)
        return false;

    game_entity::entity entity{id};
    auto geometry = entity.geometry();
    if (!geometry.is_valid() || !geometry.is_dynamic())
        return false;
