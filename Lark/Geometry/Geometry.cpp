...
inline math::v3 calculate_triangle_normal(const math::v3 &v0, const math::v3 &v1,
                                          const math::v3 &v2)
{
...
#pragma omp parallel for
    for (s32 i = 0; i < (s32)num_triangles; ++i)
    {
        ...
        const math::v3 n =
            calculate_triangle_normal(m.positions[i0], m.positions[i1], m.positions[i2]);
...
u64 get_scene_size(const scene &scene)
{
    ...
    {
        u64 lod_size{su32 + lod.name.size() +
                     su32}; // LOD name length, name string, number of meshes
 ...
 * @param settings     Geometry import settings (controls normal/tangent recalculation, smoothing
 * angle, etc.).
 *
 * @return true if successful, false otherwise (e.g., invalid indices, size mismatch).
 */
bool update_scene_mesh_positions(scene &scn, size_t lod_index, size_t mesh_index,
