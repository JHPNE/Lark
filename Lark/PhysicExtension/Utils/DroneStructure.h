...
struct InertiaProperties
{
    ...
    {
        Matrix3f I;
        I << principal_inertia.x(), product_inertia.x(), product_inertia.z(), product_inertia.x(),
            principal_inertia.y(), product_inertia.y(), product_inertia.z(), product_inertia.y(),
            principal_inertia.z();
        return I;
    }

    [[nodiscard]] Matrix3f GetInverseInertiaMatrix() { return GetInertiaMatrix().inverse(); }

    [[nodiscard]] Vector3f GetWeight() const { return {0, 0, -mass * 9.81f}; }
};
...
struct AeroDynamicsProperties
{
    ...
    [[nodiscard]] Matrix3f GetDragMatrix() const { return parasitic_drag.asDiagonal(); }
};
...
struct RotorProperties
{
    ...
    [[nodiscard]] float GetTorqueThrustRatio() const { return k_m / k_eta; }
